    ...
    <nav className="sticky top-0 z-50 w-full border-b border-[var(--border-subtle)] bg-[var(--bg-card)]/95 backdrop-blur-md transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <img src={ZorvynLogo} alt="Zorvyn" className="w-12 h-10 sm:w-16 sm:h-12 object-contain" />
            <div className="hidden sm:flex flex-col leading-tight border-l border-slate-200 dark:border-slate-800 pl-3">
              <span className="text-xl font-black text-[var(--text-heading)] uppercase">
          ...
          <div className="flex items-center gap-3">
            {/* Theme Toggle */}
            <button onClick={toggleTheme} className="p-2.5 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-canvas)] text-amber-500">
            ...
            <div className="hidden md:flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl border border-slate-200 dark:border-slate-700">
              <button
                ...
                style={role === 'VIEWER' ? activeBtnStyle : { color: '#94a3b8' }}
              >
                <Eye size={14} /> Viewer
              </button>
              <button
            ...
            {/* Mobile Hamburger */}
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="md:hidden p-2 text-[var(--text-main)]">
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-[var(--bg-card)] border-b border-[var(--border-subtle)] p-6 space-y-4 shadow-xl">
           <button onClick={() => {setRole('VIEWER'); setIsMenuOpen(false)}} 
           ...
           <button onClick={() => {setRole('ADMIN'); setIsMenuOpen(false)}} 
